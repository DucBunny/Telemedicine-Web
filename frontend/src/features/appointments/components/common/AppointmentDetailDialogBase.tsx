...
export const AppointmentDetailDialogBase = ({
  isOpen,
  onOpenChange,
  appointment,
  role,
  ...
  const personName = isPatientView
        ...
        {isCancelled ? (
          <DialogHeader className="border-b border-gray-100 p-3 md:p-5 md:pb-4">
            <div className="flex flex-col items-center">
              ...
              <DialogDescription>Lịch khám này đã bị hủy.</DialogDescription>
            </div>
          </DialogHeader>
        ) : (
          ...
          {isCancelled ? (
            <div className="rounded-xl border border-red-100 bg-red-500/5 p-3 md:p-4">
              ...
              <p className="text-sm leading-relaxed text-gray-900">
                {appointment.cancelReason || 'Không có lý do hủy'}
              </p>
            </div>
          ) : (
            <div>
              ...
              <p className="border-teal-primary/70 rounded-xl border bg-teal-500/5 p-3 text-sm leading-relaxed text-gray-900 md:p-4">
                {appointment.reason}
              </p>
            </div>
          )}

          <div className="flex flex-col gap-2 pt-2">{footer}</div>
        </div>
      </DialogContent>
    </Dialog>
  )
